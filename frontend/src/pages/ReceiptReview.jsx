...
function ReceiptReviewPage() {
  ...
  useEffect(() => {
    const currentPaymentId = items.find((item) => item.ticket_id === selectedId)?.payment_id;
    setManualPaymentId(currentPaymentId && currentPaymentId !== "queued" ? currentPaymentId : "");
  ...
  async function savePaymentId() {
    if (!selectedId || !canEditPaymentId || !PAYMENT_ID_PATTERN.test(manualPaymentId)) return;
    ...
    try {
      ...
      // A successful replacement cannot retain the old payment's conflict
      // metadata because the backend rejects IDs already used by another
      // ticket.
      setItems((currentItems) => currentItems.map((item) => item.ticket_id === (payment.ticket_id || selectedId) ? mergePaymentUpdate(item, { ...payment, conflict: null }) : item));
      setConflictRefreshVersion((current) => current + 1);
      setManualPaymentId(payment.payment_id || manualPaymentId);
      setMessage(detail?.payment_id === "queued" ? "Payment ID saved." : "Payment ID corrected.");
  ...
  const canEditPaymentId = Boolean(detail?.status === "NotVerified" && detail?.s3_url && detail?.payment_id != null);
